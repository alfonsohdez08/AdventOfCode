    ...
    internal static class Day3
    {
        ...
        public static (int part1, int part2) Solve(bool runTestCase = true)
        {
            ...
            for (int lineIdx = 0; lineIdx < lines.Length; lineIdx++)
            {
                ...
                {
                    int startIdx = idx;
                    // advances pointer to the non last digit
                    while (idx < line.Length && char.IsDigit(line[idx]))
                    {
                        idx++;
                    }
                    
                    // if indices are different, it means there's a least one digit within the string window
                    if (startIdx != idx)
                    {
                        var isPartNumber = false;
                        for (int i = startIdx; i < idx && !isPartNumber; i++)
                        {
                            foreach (var (character, row, col) in GetAdjacentChars(lines, lineIdx, i))
                            {
                        ...
                        {
                            // part 1
                            var number = int.Parse(line.Substring(startIdx, idx - startIdx));
                            sum += number;

                            // part 2
                            for (int i = startIdx; i < idx; i++)
                            {
                                indexPartNumber[$"{lineIdx}_{i}"] = number;
                            }
                        }
                    }else
                    {
            ...
            for (int lineIndex = 0; lineIndex < lines.Length; lineIndex++)
            {
                ...
                for (int i = 0; i < line.Length; i++)
                {
                    if (line[i] == '*')
                    {
                        var (partNumbers, count) = GetAdjacentPartNumbers(lines, lineIndex, i, indexPartNumber);
                        if (count == 2)
                        {
                            // hack to cover case where part number are equals
                            if (partNumbers.Length != count)
                            {
                                partNumbers = new int[] { partNumbers[0], partNumbers[0] };
                            }
        ...
        private static int QueryPartNumber(Dictionary<string, int> indexPartNumber, int row, int col)
        {
            string key = $"{row}_{col}";
            return indexPartNumber.TryGetValue(key, out int value) ? value : -1;
        }

        private static (int[] partNumbers, int count) GetAdjacentPartNumbers(string[] lines, int lineIndex, int index, Dictionary<string, int> indexPartNumber)
        {
            ...
            if (lineIndex - 1 >= 0)
            {
                int topPartNumberCount = 0;
                if (index - 1 >= 0 && QueryPartNumber(indexPartNumber, lineIndex - 1, index - 1) != QueryPartNumber(indexPartNumber, lineIndex - 1, index))
                {
                    topPartNumberCount++;
                }

                if (index + 1 < lines[0].Length && QueryPartNumber(indexPartNumber, lineIndex - 1, index) != QueryPartNumber(indexPartNumber, lineIndex - 1, index + 1))
                {
                    topPartNumberCount++;
                }

                // adjustment
                if ((topPartNumberCount == 0 || topPartNumberCount == 2) && QueryPartNumber(indexPartNumber, lineIndex - 1, index) != -1)
                {
                    // top middle
                    if (topPartNumberCount == 2)
                    {
                        topPartNumberCount--;
                    }
                    else // edge case where top row is a part number
                    {
                        topPartNumberCount++;
                    }
                }

                partNumberCount += topPartNumberCount;
            }

            // checking middle left
            if (index - 1 >= 0 && QueryPartNumber(indexPartNumber, lineIndex, index - 1) != -1)
            {
                partNumberCount++;
            }

            // checking middle right
            if (index + 1 < lines[0].Length && QueryPartNumber(indexPartNumber, lineIndex, index + 1) != -1)
            {
                partNumberCount++;
            }
            ...
            {
                int bottomPartNumberCount = 0;

                if (index - 1 >= 0 && QueryPartNumber(indexPartNumber, lineIndex + 1, index - 1) != QueryPartNumber(indexPartNumber, lineIndex + 1, index))
                {
                    bottomPartNumberCount++;
                }
                ...
                // adjustment
                if ((bottomPartNumberCount == 0 || bottomPartNumberCount == 2) && QueryPartNumber(indexPartNumber, lineIndex + 1, index) != -1)
                {
                    // bottom middle
                    if (bottomPartNumberCount == 2)
                    {
                        bottomPartNumberCount--;
                    }
                    else // edge case where bottom row is a part number
                    {
                        bottomPartNumberCount++;
                    }
                }

                partNumberCount += bottomPartNumberCount;
            }

            var partNumbers = new List<int>();
            foreach (var (character, row, col) in GetAdjacentChars(lines, lineIndex, index))
            {
                if (char.IsDigit(character) && indexPartNumber.ContainsKey($"{row}_{col}"))
                {
                    partNumbers.Add(indexPartNumber[$"{row}_{col}"]);
                }
            }

            return (partNumbers.Distinct().ToArray(), partNumberCount);
        }

        private static IEnumerable<(char character, int row, int col)> GetAdjacentChars(string[] lines, int lineIndex, int index)
        {
            ...
            if (lineIndex - 1 >= 0)
            {
                ...
                if (index - 1 >= 0)
                {
                    yield return (lines[lineIndex - 1][index - 1], lineIndex - 1, index - 1);
                }

                // top
                yield return (lines[lineIndex - 1][index], lineIndex - 1, index);
            ...
            if (index - 1 >= 0)
            {
                yield return (lines[lineIndex][index - 1], lineIndex, index - 1);
            }
